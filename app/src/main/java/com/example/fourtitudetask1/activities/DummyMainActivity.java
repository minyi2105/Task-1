...
public class DummyMainActivity extends AppCompatActivity implements View.OnClickListener, AsyncResponse {
    ...
    public static void start(Context context) {
        Intent starter = new Intent(context, DummyMainActivity.class);
    ...
    protected void onResume() {
        ...
        DummyDbUtil.GetAllDummies asyncTask = new DummyDbUtil.GetAllDummies(DummyMainActivity.this);
    ...
    public void processFinish(List<Dummy> dummies) {
        if (dummies.size() > 0) {
            DummyAdapter dummyAdapter = new DummyAdapter(DummyMainActivity.this, rvDummy, cvDummy, dummies);
            rvDummy.setAdapter(dummyAdapter);
            dummyAdapter.notifyDataSetChanged();
        } else {
            DummyDbUtil.AddAllDummies asyncTask = new DummyDbUtil.AddAllDummies(DummyMainActivity.this, loadDummyList());
            //this to set delegate/listener back to this class
            asyncTask.delegate = this;
            asyncTask.execute();
        }
